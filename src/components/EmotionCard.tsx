...
export default function EmotionCard(props: EmotionCardProps) {
    ...
    const emotionImage = props?.emotionTitle
        ? emotionImages[props.emotionTitle.toLowerCase()] || defaultEmotion
        : defaultEmotion;
    ...
    return (
        <Card elevation={10} sx={{ display: 'flex', alignItems: 'center', borderRadius: '30px' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                <CardContent sx={{ flex: '1 0 auto', padding: '8px !important' }}>
                    <Typography component="div" variant="subtitle1">
                        {props?.emotionTitle ? 'Lately, has been feeling' : 'No emotion detected'}
                    </Typography>
                    <Typography variant="h6" component="div">
                        {props?.emotionTitle || 'No emotion detected'}
                    </Typography>
                </CardContent>
            </Box>
        </Card>
